...
var options = 	{
					...
					chaincode:{
				            zip_url: 'https://github.com/pierodel/learn-chaincode/archive/v2.0.zip', //http/https of a link to download zip
				            unzip_dir: 'learn-chaincode-2.0/account',                                    //name/path to folder that contains the chaincode you want to deploy (path relative to unzipped root)
				            git_url: 'https://github.com/pierodel/learn-chaincode/account',        //GO get https URL. should point to the desired chaincode repo AND directory
				        }
				};
...
function hfc_load_peers(){
	var chain = hfc.newChain("targetChain_test");
	...
	// Add a peer's URL
	chain.addPeer(peers[0]);
	chain.enroll(prefer_type1_users(users));
...
function check_if_deployed(e, attempt){
	if(e){
		...
		chaincode.query.read(['_accountindex'], function(err, resp){
