...
$(document).on('ready', function() {
	...
	$('#submit').click(function(){
		...
		var obj = 	{
						type: 'create',
						from: $('select[name="accountNumFrom"]').val(),
						to: $('select[name="accountNumTo"]').val(),
						amount: $('input[name="amount"]').val(),
						v: 1
					};
		if(obj.from && obj.to && obj.amount ){
			console.log('creating transaction, sending', obj);
	...
	function showHomePanel(){
		...
		setTimeout(function(){
			// $('#user1wrap').html('Value is :');											//reset the panel
			// $('#user2wrap').html('Value is :');	
...
function connect_to_server(){
	...
	function onMessage(msg){
		try{
			...
			console.log("message obj", msgObj.marble)
			if(msgObj.marble){
				console.log('rec', msgObj.msg, msgObj);
				if (msgObj.marble.accountno=="01-000-3130-0000-000"){
					$('#user1wrap').html('Value is : '+ Number(msgObj.marble.balance.toString()).toFixed(2)+ ' '+ msgObj.marble.currency);											//reset the panel

				}
				else if (msgObj.marble.accountno=="01-000-4130-0000-000"){
					$('#user2wrap').html('Value is : '+ Number(msgObj.marble.balance.toString()).toFixed(2)+ ' '+ msgObj.marble.currency);				
				}
			}
