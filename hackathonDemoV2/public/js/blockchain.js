...
function new_block(newblck){									//rec a new block
	console.log("NEWBLOCK", blocks[Number(newblck.id)])
	if(!blocks[Number(newblck.id)]){
		console.log("entered with new block id");
...
function formatPayload(str, ccid){								//create a sllliiiggghhhtttlllllyyy better payload name from decoded payload
	console.log(ccid);
	console.log(str);
	var func = ['init', 'delete', 'write', 'init_account', 'transfer_balance', 'open_trade', 'perform_trade', 'remove_trade'];
